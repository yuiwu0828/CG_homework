...
//		init
		...
		disk_model = glm::mat4(1.0f);
		cat_model = glm::mat4(1.0f);
		teapotdisk_model = glm::mat4(1.0f);
		teapot_model = glm::mat4(1.0f);
		ball_model = glm::mat4(1.0f);
//		base
		base_model = glm::rotate(base_model, glm::radians(-0.2f * i), glm::vec3(0, 1, 0));
//		disk
//		disk_model = glm::rotate(base_model, glm::radians(-0.5f * i * cat_speed), glm::vec3(0, 1, 0));
		disk_model = glm::rotate(disk_model, glm::radians(last_disk - (0.5f * cat_speed)), glm::vec3(0, 1, 0));
		last_disk -= 0.5f * cat_speed;
		disk_model = glm::translate(disk_model, glm::vec3(0.0, 1.3, 0.0));
//		teapotdisk
		teapotdisk_model = glm::translate(base_model, glm::vec3(-1.0, 1.5, 0.0));
		teapotdisk_model = glm::rotate(teapotdisk_model, glm::radians(-1.0f * i), glm::vec3(0, 1, 0));
//		teapot
		teapot_model = teapotdisk_model;
//		cat
		cat_model = glm::translate(disk_model, glm::vec3(2.5, 0.0, 0.0));
//		ball
		ball_model = glm::translate(cat_model, glm::vec3(0.0, 0.5, 2.0));
		ball_model = glm::rotate(ball_model, glm::radians(1.2f * i), glm::vec3(1, 0, 0));
//		scale
		base_model = glm::scale(base_model, glm::vec3(1.25, 1.0, 1.25));
		disk_model = glm::scale(disk_model, glm::vec3(4.0, 1.0, 4.0));
		teapot_model = glm::scale(teapot_model, glm::vec3(0.3, 0.3, 0.3));
		ball_model = glm::scale(ball_model, glm::vec3(1.0, 1.0, 1.0));
		cat_model = glm::scale(cat_model, glm::vec3(1.0, 1.0, 1.0));
		teapotdisk_model = glm::scale(teapotdisk_model, glm::vec3(1.2, 1.2, 1.2));


		DrawModel("base", base_model, view, projection);
		DrawModel("disk", disk_model, view, projection);
		DrawModel("disk", teapotdisk_model, view, projection);
		DrawModel("cat", cat_model, view, projection);
		DrawModel("ball", ball_model, view, projection);
		DrawModel("teapot", teapot_model, view, projection);

		if (i == 3600) i = 0; 
		else i = i + 1;
		if (last_disk < -360.0) last_disk += 360.0;
